<div align="center">
  <img src="https://vercel.app👋&fontSize=40&animation=fadeIn" width="100%" />
</div>

<div align="center">
  <h3>🚀 Junior Developer | Self-Taught Developer</h3>
  <p>Self-taught developer building personal projects to broaden my knowledge and master modern technologies.</p>
</div>

---

### 💫 About Me

- 🔭 **Current Focus**: Working on building solid apps across multiple tech domains.
- 🌱 **Learning Path**: Diving deeper into **Python** and **C** programming languages.
- 👯 **Collaboration**: Open to contributing to innovative **open-source developer tooling**.
- ⚡ **Fun Fact**: I can make a solid front-end app but not the back-end work

---

### 🛠️ Tech Stack & Skills

<div align="left">
  <!-- Languages -->
  <img src="https://img.shields.io/badge/python-yellow?logo=python" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <!-- Frameworks & Libs -->
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <!-- Databases & Cloud -->
  <img src="https://shields.io" />
  <img src="https://shields.io" />
  <img src="https://shields.io" />
</div>

---

### 📊 Git Metrics & Performance

<div align="center">
  <table border="0">
    <tr>
      <td>
        <img src="https://vercel.app" alt="GitHub Stats" height="180" />
      </td>
      <td>
        <img src="https://vercel.app" alt="Top Languages" height="180" />
      </td>
    </tr>
  </table>
  
  <br />
  
  <img src="https://herokuapp.com" alt="GitHub Streak" width="100%" />
</div>

---

### 💻 Recent Open Source Contributions
<!-- This section automatically updates if you want to use github actions, or you can write your top projects manually below -->
* 🌟 [Project-Name](https://github.com) - A high-performance REST engine built with Node.js and Redis cache layer.
* 📦 [Library-Name](https://github.com) - Custom lightweight state management hook library designed for scalable React projects.

<div align="center">
  <img src="https://vercel.app" width="100%" />
</div>
